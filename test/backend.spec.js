...
describe('chained backend', () => {
  ...
  describe('all backends fail', () => {
    ...
    it('should not signal retry to i18next by default when all backends fail', (done) => {
      backend.read('fr', 'test', function(err, retry) {
        expect(err).to.be.an(Error);
        expect(err.message).to.eql('non of the backend loaded data');
        expect(retry).to.eql(false);
        done();
      });
    });

    it('should signal retry to i18next when all backends fail and retryOnFailure is enabled', (done) => {
      retryingBackend.read('fr', 'test', function(err, retry) {
        expect(err).to.be.an(Error);
        expect(err.message).to.eql('non of the backend loaded data');
